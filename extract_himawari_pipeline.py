...
def process_and_save(datetime_target, verbose=False):
    ...
    try:
        ...
        # 4. Prepare Tensor (ordered: B13, B08, B10, B14)
        ordered_bands = ['B13', 'B08', 'B10', 'B14']
        stacked_data = []
        
        for band in ordered_bands:
            if band not in local_scene:
                raise Exception(f"Band {band} not in resampled scene")
            norm = normalize_channel(local_scene[band], band, verbose=verbose)
            stacked_data.append(norm)
        
        if len(stacked_data) != 4:
            raise Exception(f"Expected 4 bands, got {len(stacked_data)}")
        ...
        # Validate
        if np.all(tensor == 0):
            raise Exception("All tensor values are zero - data quality issue")
        ...
        # Save .png (B13 IR channel inverted for visualization)
        viz_data = 1.0 - tensor[:, :, 0]
        png_path = save_dir / f"{file_prefix}.png"
        plt.imsave(png_path, viz_data, cmap='gray')
        
        if verbose:
            stats = f"min={np.min(tensor):.3f} max={np.max(tensor):.3f} mean={np.mean(tensor):.3f}"
            print(f"  Saved: {tensor.shape} {stats}")
        
    except Exception as e:
        raise Exception(f"Processing failed: {e}")
        
    finally:
        # Cleanup temp files for this timestamp
...
if __name__ == "__main__":
    ...
    process_and_save(target_time, verbose=True)
    print("Done.")
